...
<style>
    ...
    .actions-sidebar { background: var(--card-light); border-radius: 24px; padding: 32px; border: 1px solid var(--border-light); position: sticky; top: 100px; width: 320px; flex-shrink: 0; }
    ...
    .form-group input, .form-group textarea { width: 100%; padding: 12px; border-radius: 10px; border: 1px solid var(--border-light); background: var(--bg-light); font-family: 'Tajawal', sans-serif; }
    body.dark-mode .form-group input, body.dark-mode .form-group textarea { background: var(--bg-dark); border-color: var(--border-dark); }
...
<script>
    ...
    async function fetchDetails() {
        try {
            const response = await fetch(`/api/warehouse/store-returns/${returnId}`, {
    ...
    function displayDetails(data) {
        ...
        document.getElementById('marketerName').textContent = data.marketer_name || '-';
        ...
        if (data.status === 'rejected' && data.notes) {
            ...
            document.getElementById('rejectedBy').textContent = data.keeper_name || 'غير محدد';
    ...
    function showApproveForm() {
        const html = `
            <div class="form-group">
                <label>صورة الفاتورة المختومة *</label>
                <input type="file" id="stampedImage" accept="image/*" required>
            </div>
    ...
    async function approveReturn() {
        const fileInput = document.getElementById('stampedImage');
        if (!fileInput.files[0]) {
            alert('يرجى رفع صورة الفاتورة المختومة');
            return;
        }
        ...
        try {
            const response = await fetch(`/api/warehouse/store-returns/${returnId}/approve`, {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' },
                body: formData
            });
            ...
            if (response.ok) {
                ...
                modal.innerHTML = `
                    <div style="background: var(--card-light); border-radius: 20px; padding: 40px; max-width: 400px; text-align: center;">
                        ...
                        <h3 style="font-size: 24px; font-weight: 800; margin-bottom: 12px; font-family: 'Tajawal', sans-serif;">تم توثيق الإرجاع بنجاح</h3>
                ...
                alert(result.message || 'فشل التوثيق');
            }
        } catch (error) {
    ...
    async function rejectReturn() {
        ...
        try {
            const response = await fetch(`/api/warehouse/store-returns/${returnId}/reject`, {
                method: 'PUT',
                headers: { 
            ...
            if (response.ok) {
                ...
                modal.innerHTML = `
                    <div style="background: var(--card-light); border-radius: 20px; padding: 40px; max-width: 400px; text-align: center;">
                        <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2" style="margin: 0 auto 20px;"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
                        <h3 style="font-size: 24px; font-weight: 800; margin-bottom: 12px; font-family: 'Tajawal', sans-serif;">تم رفض الإرجاع بنجاح</h3>
                        <button onclick="window.location.reload()" style="margin-top: 24px; padding: 12px 32px; background: var(--primary); color: white; border: none; border-radius: 12px; font-weight: 700; cursor: pointer; font-family: 'Tajawal', sans-serif;">حسناً</button>
                    </div>
                `;
                document.body.appendChild(modal);
