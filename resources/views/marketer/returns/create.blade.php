...
@section('title', 'طلب إرجاع بضاعة جديد')
...
<div class="page-header">
    <h1 class="page-title">طلب إرجاع بضاعة جديد</h1>
    <p class="page-subtitle">قم بإضافة المنتجات والكميات المراد إرجاعها</p>
</div>
...
<div class="form-actions">
    <button type="button" class="btn btn-secondary" onclick="window.location.href='/marketer/returns'">إلغاء</button>
    <button type="button" class="btn btn-primary" onclick="submitReturn()">إرسال طلب الإرجاع</button>
</div>
@endsection
    ...
    async function fetchProducts() {
        try {
            const [productsRes, stockRes] = await Promise.all([
                ...
                fetch('/api/marketer/stock/actual', {
                    headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' }
                })
            ]);
            ...
            if (products.length === 0) {
                showAlert('لا توجد منتجات في مخزونك الفعلي للإرجاع', 'error');
            } else {
                addProductRow();
            }
        } catch (error) {
    ...
    function addProductRow() {
        ...
        row.innerHTML = `
            <select class="product-select" data-id="${id}" onchange="updateMaxQuantity(${id})">
                ...
                ${products.map(p => `<option value="${p.id}" data-stock="${p.actual_quantity}">${p.name} - ${p.current_price} دينار (متوفر: ${p.actual_quantity})</option>`).join('')}
            </select>
    ...
    async function submitReturn() {
            ...
            if (productSelect && quantityInput) {
                ...
                if (productId && quantity > 0) {
                    if (maxStock && quantity > maxStock) {
                        showAlert(`الكمية المطلوبة أكبر من المتوفر في مخزونك (الحد الأقصى: ${maxStock})`, 'error');
                        return;
                    }
        ...
        try {
            ...
            if (response.ok) {
                ...
                setTimeout(() => window.location.href = '/marketer/returns', 1500);
            } else {
                showAlert(result.message || 'حدث خطأ أثناء إنشاء طلب الإرجاع', 'error');
            }
        } catch (error) {
            console.error('Error:', error);
            showAlert('حدث خطأ أثناء إرسال طلب الإرجاع', 'error');
        }
    }
