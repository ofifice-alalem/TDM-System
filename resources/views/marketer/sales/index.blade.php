...
<style>
    ...
    .btn-add { padding: 12px 24px; background: var(--primary); color: white; border: none; border-radius: 12px; font-weight: 700; cursor: pointer; display: flex; align-items: center; gap: 10px; text-decoration: none; font-family: 'Tajawal', sans-serif; }
    ...
    .filters-bar { background: var(--card-light); padding: 24px; border-radius: 16px; margin-bottom: 24px; display: grid; grid-template-columns: 1fr 1fr; gap: 20px; border: 1px solid var(--border-light); }
    ...
    .invoice-icon { width: 52px; height: 52px; border-radius: 12px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
    .invoice-info { flex: 1; display: grid; grid-template-columns: 1.5fr 1fr 1fr 1fr; align-items: center; gap: 20px; }
...
<div class="page-header">
    ...
    <a href="/marketer/sales/create" class="btn-add">
...
<script>
    ...
    async function fetchInvoices() {
        try {
            ...
            document.getElementById('invoicesList').innerHTML = '<div class="empty-state">⚠️ <h3>خطأ في تحميل البيانات</h3></div>';
        }
    }
        ...
        container.innerHTML = filtered.map(inv => {
            ...
            return `
                <div class="invoice-card">
                    <div class="invoice-icon" style="background: ${status.bg}; color: ${status.color};">${status.icon}</div>
