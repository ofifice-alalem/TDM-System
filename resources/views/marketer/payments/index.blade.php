...
<style>
    ...
    .btn-add-payment { padding: 12px 24px; background: var(--primary); color: white; border: none; border-radius: 12px; font-weight: 700; font-size: 15px; cursor: pointer; transition: all 0.3s ease; display: flex; align-items: center; gap: 10px; text-decoration: none; font-family: 'Tajawal', sans-serif; }
    .btn-add-payment:hover { background: #7c3aed; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(139, 92, 246, 0.3); }
    ...
    .filters-bar { background: var(--card-light); padding: 24px; border-radius: var(--card-radius); margin-bottom: 24px; display: grid; grid-template-columns: 1fr 1fr; gap: 20px; border: 1px solid var(--border-light); box-shadow: var(--shadow-sm); }
    ...
    .payment-info-group { flex: 1; display: grid; grid-template-columns: 1.5fr 1fr 1fr 1fr; align-items: center; gap: 20px; }
    .payment-num { font-size: 18px; font-weight: 800; color: var(--primary); }
    .store-info { display: flex; flex-direction: column; }
    .store-label { font-size: 11px; color: #94a3b8; font-weight: 600; }
    .store-value { font-size: 14px; font-weight: 700; color: var(--text-light); }
    body.dark-mode .store-value { color: var(--text-dark); }
    .amount-info { display: flex; flex-direction: column; }
    .amount-label { font-size: 11px; color: #94a3b8; font-weight: 600; }
...
<div class="page-header">
    ...
    <a href="/marketer/payments/create" class="btn-add-payment">
                ...
                <div class="payment-card-premium">
                    ...
                    <div class="payment-info-group">
                        ...
                        <div class="store-info">
                            <span class="store-label">المتجر</span>
                            <span class="store-value">${pay.store_name}</span>
                        </div>
                    ...
                    <a href="/marketer/payments/${pay.id}" class="btn-action">عرض التفاصيل<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14M12 5l7 7-7 7"></path></svg></a>
                </div>
            `;
        }).join('');
    }
