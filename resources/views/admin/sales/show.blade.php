...
<style>
    ...
    .modal-dialog { background: var(--card-light); border-radius: 20px; padding: 32px; max-width: 550px; width: 90%; }
    body.dark-mode .modal-dialog { background: var(--card-dark); }
    .modal-close { position: absolute; top: 16px; left: 16px; width: 32px; height: 32px; border: none; background: rgba(0, 0, 0, 0.05); border-radius: 8px; cursor: pointer; display: flex; align-items: center; justify-content: center; }
    ...
    .upload-area { border: 2px dashed #d1d5db; border-radius: 12px; padding: 40px 20px; text-align: center; margin-bottom: 20px; background: #f9fafb; cursor: pointer; }
    .upload-area:hover { border-color: #8b5cf6; background: rgba(139, 92, 246, 0.02); }
    .modal-input { width: 100%; padding: 12px 16px; border-radius: 12px; border: 1px solid var(--border-light); background: var(--bg-light); color: var(--text-light); margin-bottom: 20px; font-family: 'Tajawal', sans-serif; font-size: 14px; min-height: 100px; }
...
<div class="page-layout">
    ...
    <div class="invoice-content">
        <div class="info-card">
            <div class="info-grid">
                <div><div class="info-label">المسوق</div><div class="info-value" id="marketerName">-</div></div>
...
<script>
            ...
            if (needsFile) {
                content.innerHTML = `
                    <div class="upload-area" id="uploadArea">
                        ...
                        <div style="font-size: 16px; font-weight: 600; color: var(--text-light); margin-top: 16px;" id="uploadText">رفع صورة الفاتورة الموقعة</div>
                ...
                content.innerHTML = `
                    <p class="modal-message">يرجى إدخال سبب الرفض:</p>
                    <textarea id="rejectNotes" class="modal-input" placeholder="اكتب السبب..."></textarea>
                `;
            } else {
    ...
    async function fetchDetails() {
        try {
            const response = await fetch(`/api/admin/sales/${invoiceId}`, {
    ...
    function displayDetails(invoice, items) {
        ...
        document.getElementById('marketerName').textContent = invoice.marketer_name || '---';
    ...
    function renderActions(status, stampedImage) {
        ...
        if (status === 'pending') {
            html += `
                <button class="btn btn-success" onclick="alert('لا يمكن للإدارة توثيق الفواتير')" disabled style="opacity: 0.5; cursor: not-allowed;">
                ...
                <button class="btn btn-danger" onclick="alert('لا يمكن للإدارة رفض الفواتير')" disabled style="opacity: 0.5; cursor: not-allowed;">
    ...
    async function approveInvoice() {
        const imageFile = await showModal('توثيق الفاتورة', 'يرجى رفع صورة الفاتورة المختومة', true);
        if (!imageFile || imageFile === true) return;

        const formData = new FormData();
        formData.append('stamped_invoice_image', imageFile);

        try {
            const response = await fetch(`/api/warehouse/sales/${invoiceId}/approve`, {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' },
                body: formData
            });
            ...
            if (response.ok) {
                await showModal('نجح', 'تم توثيق الفاتورة بنجاح');
    ...
    async function rejectInvoice() {
        const notes = await showModal('رفض الطلب', 'REJECT');
        if (!notes || notes === true) return;

        try {
            const response = await fetch(`/api/warehouse/sales/${invoiceId}/reject`, {
                method: 'PUT',
                headers: { 
            ...
            if (response.ok) {
                await showModal('نجح', 'تم رفض الطلب بنجاح');
                window.location.reload();
            } else {
                await showModal('خطأ', 'فشل رفض الطلب');
            }
        } catch (error) {
            await showModal('خطأ', 'حدث خطأ');
        }
    ...
    async function fetchRejectionDetails() {
        try {
            const response = await fetch(`/api/admin/sales/${invoiceId}/rejection`, {
                headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' }
            });
            const result = await response.json();
            if (result.data) {
